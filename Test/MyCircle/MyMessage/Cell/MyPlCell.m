...
@interface MyPlCell ()<TTTAttributedLabelDelegate>
@property (nonatomic, strong) UIView *lineView;
@property (nonatomic, strong) UIButton *hfButton;
...
- (void)createWJTableViewCellView
{
    ...
    _hfButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _hfButton.frame = CGRectMake(WIDTH - 60 - 15, 20, 60, 28);
    [_hfButton setImage:[UIImage imageNamed:@"hf.png"] forState:UIControlStateNormal];
    [_hfButton addTarget:self action:@selector(OnClickHfButton:) forControlEvents:1<<6];
    [self.contentView addSubview:_hfButton];
...
- (void)setModel:(MyPlModel *)model
{
    ...
    self.plContent.text = [NSString stringWithFormat:@"回复您:%@",model.content];
    ...
    if ([PublicModel isBlankString:model.to_member_id])
    {
        _aLable.hidden = YES;
    }
    else
    {
        _aLable.hidden = NO;
        NSString *text = [NSString stringWithFormat:@"@%@:%@",model.to_member_name,model.to_content];
        NSUInteger length = model.to_member_name.length;
        
        NSRange nameRange = {0,(int)(length + 1)};
...
- (void)layoutSubviews
{
    ...
    if (![PublicModel isBlankString:self.model.to_member_id])
    {
        _aLable.frame = CGRectMake(15,cellY - 2, WIDTH - 30, 15);
        cellY = cellY + 23;
    }
...
- (void)OnClickHfButton:(UIButton *)hfButton
{
//    NSLog(@"点击了回复");
